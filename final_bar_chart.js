...
const marginBar = {top: 20, right: 50, bottom: 70, left: 100}
...
function drawBarChart(finalMapData, top_15, selectedYearLabel) {
    ...
    if(!barChartCheckboxMarkup[selectedYearLabel]){
        barChartCheckboxMarkup[selectedYearLabel] = "";
        allCrimesBar.forEach((i, idx) =>{
            if(idx < 13 && idx !== 1 && idx!==3){
                barFilterData[i]=true
                barChartCheckboxMarkup[selectedYearLabel] += `<span class="bar_checkbox_span"><input type="checkbox" onclick="drawBarEvt()" id="${i}-checkbox" class="bar-checkbox" checked value="">
                       <label>${i}</label></span>`
            }
            else{
                barChartCheckboxMarkup[selectedYearLabel] += `<span class="bar_checkbox_span"><input type="checkbox" onclick="drawBarEvt()" id="${i}-checkbox" class="bar-checkbox" value="">
                       <label>${i}</label></span>`
                barFilterData[i]=false
            }
            barChartCheckboxMarkup[selectedYearLabel] += "<br>"
        });
    }

    $('#bar_checkbox').html(barChartCheckboxMarkup[selectedYearLabel])
...
function drawBarViz(barFilterData){
    ...
    const yAxisCall = d3.axisLeft(yAxisRange)
        .ticks(6)
        .tickFormat(d => d == 0 ? 0 : numberFormat(d))
    ...
    rectsBar.enter().append("rect")
        ...
        .on("mousemove", function(event, d){
            barTooltip
                ...
                .html("Crime: " + (d.crime_type) + "<br>Total Cases reported: "+ numberFormat(d.data));
        })
...
function drawBarEvt(){
    ...
    if(count>13){
        ...
        barFilterData[checkbox_id]= false;
        $('.modal-body').html('<p>You can select atmost 13 crime types for bar chart.</p><p>Please unselect other crime types.</p><button id="close_button" style="float: right;" type="submit" class="btn btn-warning" onclick="closeModal()">Close</button>')
        $("#myModal").modal('show');
        return;
    }
