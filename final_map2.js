...
let map2Svg = d3.select(".map2-svg")
...
function drawMap2(map2FilterData){
        ...
        if(value == true){
            groupData.get(key).forEach((row)=>{
                ...
                if(typeof(row.date) === "string")
                    ...
                    sub.push(row.date.toISOString().split('T')[0])
                sub.push(row.time)
    ...
    circles.selectAll("circle")
        ...
        .on("mouseover",function(d,i){
            ...
            let x_coord = event.pageX
            let  y_coord = event.pageY
            if(x_coord < 250 && y_coord < 3200){
                x_coord += 50
            }
            else if(x_coord < 240 && y_coord < 3300){
                x_coord += 20
                y_coord -= 20
            }
            if(x_coord < 275){
            ...
            else if(y_coord > 3200 && x_coord < 500){
                x_coord += 50
                y_coord += 100
            }
            else{
